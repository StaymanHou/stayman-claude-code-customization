...
- If coming from defer (I9): confirm the `## Codify — Deferred` section is present with reasoning and that the SURFACE→task:plan entry is in `workflow-system/state/backlog.md`
...
### 4. Archive
- Move the incident report to `workflow-system/state/archive/`
...
Log to `workflow-system/state/backlog.md` targeting `task:plan`:
```markdown
## SURFACE-<timestamp>
...
Log to `workflow-system/state/backlog.md` targeting `feature:spec` with similar format.
