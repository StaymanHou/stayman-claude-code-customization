...
- **Identify the feature's earliest commit.** Run `git log --reverse --format=%H main..HEAD 2>/dev/null | head -1` to find the first commit of this feature branch; if that returns empty (feature is on `main`), find the earliest commit referencing this feature's WIP file: `git log --format=%H --reverse -- workflow-system/state/wip/<feature-name>.md workflow-system/state/archive/<feature-name>.md 2>/dev/null | head -1`. Capture as `BASE_SHA`.
...
1. `<feature-name>` — from the WIP filename (`workflow-system/state/wip/<feature-name>.md`).
...
6. `<wip-path>` — `workflow-system/state/wip/<feature-name>.md`.
...
- **Mode 3: auto-backlog with prominent chat surface.** Append each MAJOR finding to `workflow-system/state/backlog-quality-findings.md` as a new `## SURFACE-<date>-QUALITY-<short-slug>` entry (priority: medium) under the `# <feature-name> — <today YYYY-MM-DD>` section (create the section if absent; create the file with the canonical header paragraph if absent — see `workflow-system/state/backlog-quality-findings.md` top-of-file for the shape). Then append **exactly one** pointer entry to `workflow-system/state/backlog.md` per feature: `## Code-quality findings — <feature-name> (<YYYY-MM-DD>)` with `**Pointer:**` (count + 1-line summary + `[`workflow-system/state/backlog-quality-findings.md`](backlog-quality-findings.md)` reference), `**Priority:**`, `**Status:** pending`, `**Pickup shape:**` lines mirroring the existing block at `backlog.md` (the close-commit-discipline pointer is the canonical shape). **Idempotency on re-run:** if a `# <feature-name> — <today>` section already exists in the findings file (e.g. F40 back-loop after refactor re-runs review-quality), append the new finding blocks under that existing section and update the pointer entry's count + summary inline rather than creating a duplicate pointer. Print the findings in chat (one line each) with the heading: "MAJOR findings auto-backlogged to `workflow-system/state/backlog-quality-findings.md` per drive_mode=autopilot; pointer added to `workflow-system/state/backlog.md`. To address now, run `/feature-refactor`; to dismiss, edit the WIP's `## Code-Quality Review` section and mark with `[DISMISSED]`." Emit `TRANSITION: F39`.

**Case C — `n_critical == 0` AND `n_major == 0` AND `n_minor >= 1`:**
- Modes 2-3: auto-backlog each MINOR finding to `workflow-system/state/backlog-quality-findings.md` (priority: low), grouped under a `# <feature-name> — <today YYYY-MM-DD>` section (create the section if absent; create the file with the canonical header paragraph if absent — see `workflow-system/state/backlog-quality-findings.md` top-of-file for the shape). Then append **exactly one** pointer entry to `workflow-system/state/backlog.md` per feature: `## Code-quality findings — <feature-name> (<YYYY-MM-DD>)` with `**Pointer:**` (count + 1-line summary + `[`workflow-system/state/backlog-quality-findings.md`](backlog-quality-findings.md)` reference), `**Priority:** low (all)`, `**Status:** pending`, `**Pickup shape:**` lines mirroring the canonical shape in `backlog.md`. Idempotency on re-run: see Case B's note above (same discipline applies — append under existing section, update pointer count inline). No prompt. Print a one-line summary in chat: "N MINOR findings auto-backlogged to `workflow-system/state/backlog-quality-findings.md` under `# <feature-name> — <YYYY-MM-DD>`; pointer added to `workflow-system/state/backlog.md`. Review section persisted in WIP." Emit `TRANSITION: F39`.
