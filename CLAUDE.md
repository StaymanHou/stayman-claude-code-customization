...
- **Plan-time downstream-contract-impacts grep must include literal-payload-object assertions AND array-length assertions, not just key-name greps.** Sub-case of the "Plan-level downstream contract impacts pass" + "Cross-layer contract migration" bullets above. When a phase adds a new key to a payload-object passed to a function (e.g. `updateHash({...})`, `setState({...})`, an event dispatch, an API call payload), tests in the codebase may grep for the **literal patch-object string** — and adding a new key changes the suffix of every literal-match assertion. Key-name grep alone misses these because the assertion doesn't mention the new key. **Practical grep at plan time:** for each public-surface change that adds a key to a payload-object, in addition to grepping for the new key name, also grep for the *call site itself* in tests — e.g. `grep -RE "fn\(\{" ./tests` or `grep -F "updateHash({ view: " ./tests` — and inspect every match for literal-string assertions that would break when the payload's key set grows. Update the matching test strings as part of the same phase. Caught 2026-06-03 during v3 WP5 build: P1.2 added a `date:` key to all branches of the `updateHash({...})` hash-write dispatcher; the existing pin at `test_visualize_cli.sh:988-996` ("WP8-P2+WP7-P2+WP11-P2 codify: hash-write five-branch dispatch") greps for literal `updateHash({ view: 'X', range: null, month: null, preset: null, ranges: null })` strings — adding the `date:` key broke 4 of 5 grep checks. **Sub-case: array-length-add.** When a phase extends an array of known cardinality (tile lists, route arrays, filter-kind enumerations, schema field lists, status enums), tests downstream may assert against the array's `.length === N` or `.length >= N`. Key-name grep misses these because the assertion doesn't mention any individual element. **Practical grep at plan time, in addition to the key-name + literal-payload-object greps above:** for each array the phase is extending, grep tests for the array's name + `.length` (e.g. `grep -RE "tileIds\.length\s*===\s*\d+" ./tests`) and inspect each match for cardinality assumptions that would break when the array grows. Update the matching assertions as part of the same phase. Caught 2026-06-06 during v3 WP11 P2 codify: adding a 4th `away` tile to HeadlineCard's `tiles` array broke `WP10-P2 behavioral: HeadlineCard renders w/ 3 tiles` which asserted `tileIds.length === 3` — high-confidence obsolete-test auto-fix at triage, relaxed to `>= 3`. **5 observed instances of `SURFACE-2026-05-29-WP3-PLAN-DOWNSTREAM-CONTRACT-MISS`** across both subcases (1st = 2026-05-10 incident-codify, 2nd = 2026-05-29 WP3 P2, 3rd = 2026-05-29 WP4 P3, 4th = 2026-06-03 WP5 P1.2 [literal-payload-object subcase], 5th = 2026-06-06 WP11 P2 [array-length subcase]). Pattern is now well-established; the backlog item proposing to codify this into `feature-plan` SKILL.md as a mechanical step remains pending — until then, this convention is the doc-side fallback. Both subcases share the same prevention mechanism: before sealing a phase's impl tasks, grep tests + downstream consumers for ANY assertion against the shape being changed.
