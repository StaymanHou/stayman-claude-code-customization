...
- **Verify-self in-place fix shortcut — rule of three reached 2026-06-07; ready to formalize.** `SURFACE-2026-05-29-VERIFY-SELF-IN-PLACE-FIX-SHORTCUT-POLICY` fired correctly for the THIRD cumulative time on 2026-06-07 during verify-human-auto-skip-when-no-integration-boundary Phase 2 verify-self (F-blocked scenario reshape from `transition_id_any: [F13, F12]` to F13-driving prompt — caught via SOFT_PASS signal, fixed in-place rather than back-looping to build, audit trail in WIP `## Discoveries`). Three instances now: v3 WP3 Phase 2 (2026-05-29), v3 WP11 Phase 1 (2026-06-06), this feature (2026-06-07). The backlog entry's "rule of three" threshold is met — the policy is ready to formalize as an explicit clause in `skills/feature-verify-self/SKILL.md` §3, gated on: (a) fix is a trivial extension of the just-completed leaf, (b) re-verification runs through a fresh model invocation (subagent or fresh skill run), (c) `## Discoveries` audit-trail entry captures what was fixed and how it was re-verified. Promote the backlog item from "low-medium pending" to actionable in the next available cycle.
