...
- **v3 sub-payload routing pattern: useMemo fallback to v2 alias key (claude-time visualize WP5–WP9 transition convention).** The interactive Dashboard in `viz_render.py::_interactive_dashboard` reads multiple data sources keyed by view (Day/Week/Month/Compare/Custom). v3 routes each view's data through its corresponding sub-payload map (`day_payloads_by_iso`, `week_payloads_by_monday`, `month_payloads_by_iso`, `compare_payloads_by_preset`) while WP9 verify-codify removes the v2 alias keys (`today`, `week`, `comparison`, `months`). **The bridging pattern** is a `useMemo` that prefers the v3 sub-payload but falls back to the v2 alias when the v3 lookup misses — this lets shared render code paths (like `isDayLike` which Day + Custom share) stay un-forked during the WP5→WP9 migration. WP5 concrete: `const dayPayload = React.useMemo(() => { if (dayPayloadsByIso[dayIso]) return dayPayloadsByIso[dayIso]; return today; }, [dayIso, today]);` — for the Day view this routes through the v3 map; for the Custom view (which shares Day's `isDayLike` reads but has its own data semantics) the useMemo falls back to the `today` alias, leaving Custom-view behavior unchanged until WP9 wires it. **Application to WP6/WP7/WP8/WP9:** each WP should introduce its own analog helper with the same fallback (`weekPayload → week`, `monthPayload → months[currentMonthIso]`, etc.) so non-target views keep their existing behavior. WP9 (Custom-range) is the bridging case — it shares Day's `isDayLike` reads, so WP9 either forks the `isDayLike` branch into `isDay`-only + `isCustom`-only OR introduces a `customPayload` with the same fallback. Established 2026-06-03 during v3 WP5 ship; load-bearing finding (the useMemo's fallback was originally intended as defensive belt-and-suspenders but turned out to be the correctness mechanism for Custom-view coexistence). Active through WP9 verify-codify; obsolete after the v2 alias keys are removed.
