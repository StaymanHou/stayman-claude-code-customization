...
- **Suggested action:** After the operator's 2nd hands-on walkthrough (the batch run once all load-bearing M11 WPs land — **WP7k is the last**), capture any copy/flow feedback as a `/task-plan` (or fold into the relevant tour skill) against `skills/tutorial-getting-started/SKILL.md` + both arm skills + `skills/tutorial-product-cycle-tour/SKILL.md` (once WP7k builds it) + `onboarding-flow-spec.md` / `full-product-cycle-tour-design.md` as applicable. Resolve this SURFACE when the walkthrough is done (pass → delete per delete-on-resolve; issues → spawn the follow-up).
- **Priority:** medium
- **Status:** pending
