...
- **✅ THE OPEN DESIGN QUESTION IS ANSWERED (2026-07-27, WP7o §D) — no longer gating WP7e.** It formerly read: the "always flat, **refuse if non-empty**" rule fires the empty-cwd guard on *every replay*, and the passed-over alternative (ask + offer a subdir fallback) was cheap now and expensive after WP7e pins the copy. **The operator answered it directly:** *"refuse if non empty, offer to delete the content of the dir"* — refuse-if-non-empty **plus an offer to clear**, greenfield-only, under a four-rule destructive protocol (refuse a git repo outright · bound deletion to the cwd's contents, never `..`/`~`/above · never pass `--force` · touch nothing on decline or ambiguity). Built as WP7o Phase 3, with 8 mutation-verified assertions + a real-execution refusal test. The remaining acceptance question is narrower and is now a *judgment* call, not a design one: **did a tour offering to delete files feel wrong?** (checklist leaf `P3.verify-human.5`).
- **⚠️ SCOPE GREW 2026-07-27 — the batch is now FOUR fixes, not three.** The greenfield re-acceptance run RAN on 2026-07-27 and **reported two defects** (a first run returning as Orchestrated with the 1–4 menu shown; the second-boundary handoff fork still appearing), both traced to one root cause: the tour's state lived only in the conversation and died at `/exit`. Ratified and built as **WP7o** (commit `ccfedac`; hardening refactor `438d88e`) — the pointer now carries `tour:`/`tour_step:` and `resume_skill` points at the ARM, which is what makes the guard reachable at all. **WP7o SUPERSEDES WP7m's decision 7m.1**, whose arms-only placement the raw Session-C log proved structurally unreachable across a boundary. So the one owed run now accepts **WP7l + WP7n + WP7m + WP7o together**, and flips all four from 🔨 BUILT to ✅ SHIPPED.
- **Two things to know going into the run, neither a defect to fix:** (1) `tests/scenarios/session.yaml::S33`/`S34` are **known-failing by design** (see `SURFACE-2026-07-27-TOUR-MARKER-NOT-READ-WHEN-POINTER-DELETED`) — **if the tour works end-to-end anyway, that confirms the diagnosis**, since the primary mechanism (`resume_skill` → the arm) carries the load and the reader guards were always defense-in-depth. (2) The suite's single FAIL is the pre-existing out-of-scope `effortLevel` settings-fixture drift.
- **Priority:** medium
- **Status:** partially-resolved — **build half DONE (all FOUR fixes BUILT: WP7l+WP7n `783bdf2`, WP7m `e7e682b`, WP7o `ccfedac`+`438d88e`); §D answered; remaining open work = the one full hands-on greenfield acceptance run → then WP7e**
