...
### Design priors — consult + capture (read+write-side, cross-workflow)

`docs/product/design-priors.md` is a per-project, deterministically-loaded record of the operator's **product-design decision leans** (focus-vs-breadth, perf-vs-ship, anti-persona, …), each paired with its *why*. Two cross-workflow behaviors hang off it, **neither of which is a state-machine transition** (no new F/I/T/P IDs — this is behavior *within* existing states, same shape as the CHANGELOG-append and context-loading mechanisms above):

- **Consult (read-side):** the three planning skills that make product-design tradeoffs — `product-roadmap`, `product-wbs`, `feature-spec` — load `design-priors.md` in their opening `## Step 0` section (the entry-point `feature-spec` uses `## Step 0: Available product context`; the non-entry-point `product-roadmap`/`product-wbs` use `## Step 0: Product context + design-priors consult`, a distinct suffix so the pinned entry-point heading stays reserved) and apply five consult-weighting rules (no-prior → common sense; prior-agrees → take it; prior breaks a tie → lean+disclose; clear default *contradicts* a strong prior → propose-never-steer; **a prior only fires on the axis it governs** — the over-infer guard). A firing prior is disclosed as `[PRIOR: <slug>] leaning <x> — flag if wrong`; priors are overridable by strong common-sense evidence.
- **Capture (write-side):** six checkpoints (`product-vision`, `product-roadmap`, `product-arch`, `product-wbs`, `feature-spec`, `feature-verify-human`) plus a `session-reflect` backstop **propose** a new prior when operator input reveals a product-design tradeoff (or identity/non-goal/anti-persona) with a *transferable why*. Capture is **propose-never-auto-write** (operator reviews/enriches the why; dedup/conflict-checked). Exclusions: technical/stack tradeoffs → `arch.md` (the arch-boundary exclusion), bare facts/scope-adds → not a prior.

The canonical contract lives in `CLAUDE.snippet.md` (`## Design priors (GLOBAL)`); the schema lives in `arch.md` (File Schema: Design Priors); enforced by `tests/check-structure.sh` [Phase 13] and behavioral scenarios `tests/scenarios/product.yaml::DP-*`. Shipped 2026-06-26 by the `design-priors` feature.
...
- **2026-06-24 — Telegram notification hook removed.** The `hooks/notify-telegram.sh` harness hook (added 2026-05-06 to replace the model-driven `notify-human` skill) is gone — no longer needed. Removed: the hook script, its live `~/.claude/settings.json` wiring (`hooks.Notification` entry, `CLAUDE_TELEGRAM_BOT_TOKEN`/`CLAUDE_TELEGRAM_CHAT_ID` env, telegram curl permission), `install.sh`'s env-var reminder, the Phase 5 hook-integrity block + telegram-specific `INTENTIONAL_DIFFS` entries in `tests/check-structure.sh`, the telegram permission in `tests/fixtures/settings.json`, and the Telegram prose in `CLAUDE.md` / `README.md` / `arch.md` / `vision.md` / three SKILL.md human-pause hints. The claude-time and claudesk hooks are untouched. There is now no out-of-band alerting; human-input moments still pause the conversation and wait. Closed SURFACE-2026-06-16-TELEGRAM-NOTIFY-FIRES-ON-BACKGROUND (the spurious-ping-on-auto-background bug it described is moot with no hook). No transition ID change — this is harness wiring, not a state-machine edit.

- **2026-06-24 — Phase 7 settings-drift now FILTERS host-specific claudesk hooks instead of excluding whole events.** Removing the telegram hook exposed a latent drift: the developer's claudesk wrapper app (`com.claudesk.app[.dev]`) installs busy/idle-monitoring hooks into live `~/.claude/settings.json` on `Notification`/`Stop`/`UserPromptSubmit`, which the test fixture cannot mirror (host-specific absolute paths, mutate on claudesk's schedule). The stopgap parked all three events in `INTENTIONAL_DIFFS`, which silently disabled drift detection on the repo-owned `claude-time` hook too. Replaced with a `strip_host_specific()` pass that drops any hook GROUP whose command mentions `claudesk` from BOTH sides before diffing. After stripping, `UserPromptSubmit` reduces to the lone `claude-time` hook on both sides and is fully drift-checked again (a broken claude-time command now FAILs Phase 7 — verified); only `Notification`/`Stop` remain in `INTENTIONAL_DIFFS` (fixture empties them so tests don't fire the notification hook). Keeps the harness and the claudesk wrapper fully decoupled. Harness wiring, no transition ID change.

- **2026-06-23 — `debug-*` category third member `debug-minimal-harness` shipped.** Adds the self-driven-reproduction sidebar: when a **behavioral** fix (drag/click/focus/keyboard gesture, CLI under real argv/stdin, HTTP under a real client, a race under real concurrency) has been **handed back untested ≥2×** on the same behavior AND that behavior is **drivable in a surface the agent controls** — even when the shipping target is native (Tauri WKWebView, Electron) the same DOM/CLI/HTTP logic usually runs in a browser/process the agent can drive — the agent stops handing back, builds a minimal standalone reproduction, and drives it with **real input** (`page.mouse`, real argv, a real request), NOT synthetic dispatch (which false-passes by masking pointer-capture/focus/hit-test/re-render bugs). Two conjunctive gates at SKILL entry: (1) self-drivable surface, (2) ≥2 hand-backs on the same behavior. Emits `DEBUG-MINHARNESS-{START,SKIP,COMPLETE,INCONCLUSIVE}` + `RETURN-TO: <caller>`; no new F/I/T/P/S transition ID. Codifies `.claude/learnings/2026-06-23-test-it-yourself-first-interactive-fixes.md` (claudesk M4 WP3 drag-reorder, where a synthetic-PointerEvent hello-world false-passed while a real `page.mouse` drag exposed the capture-on-re-render root cause). Distinct from siblings: bisect needs a known-good sibling, telemetry observes the running system; this one keys on repeated hand-back + self-drivability. Affects: new `skills/debug-minimal-harness/SKILL.md`; new "Debug techniques" rows in all three orchestrator AGENTS.md; prose mentions in `skills/feature-build/SKILL.md` §4b, `skills/task-act/SKILL.md` §3b, `skills/incident-investigate/SKILL.md` §3b; the "Sidebar skills" examples list above; 7 new Phase 3c pins in `tests/check-structure.sh`; arch.md "Current sidebars" + `CLAUDE.md` debug-* section; 3 fixtures + 3 scenarios in `tests/scenarios/debug.yaml`.
