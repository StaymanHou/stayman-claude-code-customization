...
**AS-BUILT (2026-07-28) — ✅ SHIPPED.** Delivered as a **reciprocal handoff doc**, `HANDOFF-from-mccc-2026-07-28.md` at Claudesk's root — the mirror of the inbound `HANDOFF-from-claudesk-2026-07-20.md`, chosen over a backlog SURFACE because the bundle is three deliverables (two of them pure reference pointers with no action), which a one-item-per-entry SURFACE schema fragments. All three deliverables are **pointers into this repo, not copied prose** (copied prose drifts on the next edit); only the literal install/uninstall commands and the old→new path table are inlined, since those must be actionable without a second lookup.

**The WBS's "required change, not an optional note" framing was correct but mis-scoped, and the recon corrected it:** Claudesk had **already physically migrated** to `workflow-system/` (its HEAD `aacc687`), yet its own M11 `wbs.md:60` — task 2.1, still `- [ ]` **unbuilt** — still specified `docs_list` discovery over the deleted `docs/product/*.md` + `workflow/*` paths. Building it as written would have produced a **`docs_list` that finds nothing**. But because `docs_list`/`docs_read` are **unimplemented anywhere** in that repo, this is a **spec correction, not a code change** — nothing built was broken and no migration was needed.

**Deliberately NOT fixed in place.** Filed there as `SURFACE-2026-07-28-M11-DOCS-LIST-PATHS-STALE` instead, because (i) that repo's `/product-wbs` owns its own spec and editing another product's active plan silently is a heavier act than filing against it, and (ii) the correction carries a **real design question that is Claudesk's to settle** — a flat glob over `workflow-system/product/*.md` now also picks up `transitions.md` + `design-priors.md`, so flat-glob vs. curated-ordered-set is a Claudesk UX call. The SURFACE records both options plus two properties to preserve either way: `.session.md` is **gitignored but present on disk** (discovery must not assume git-tracked ⇒ discoverable), and the `*wbs*.md` glob is **deliberate** (it catches `shape: temporary-wbs` scratch files, so it must stay a glob).

**Most valuable content transmitted** — the onboarding spec's §4c anti-brittleness clause: the **only** stable coupling Claudesk may depend on is the command name `/tutorial-getting-started`; not the flow, steps, copy, path fork, or permission-mode instruction. Proven rather than asserted — the `acceptEdits`→`auto` change landed entirely in this repo with zero Claudesk changes. Plus two facts that save Claudesk work: the tour is **self-contained in the skill install** (the greenfield sample rides each skill's whole-dir symlink, so there is no sample-fetch step to document), and **honest framing is a structurally-pinned invariant** — invite copy must not promise a 5-minute demo.

Committed locally in both repos, **not pushed** (close-commit discipline). Closes `SURFACE-2026-07-20-CLAUDESK-ONBOARDING-DESIGN`, the last of the five inbound items.
