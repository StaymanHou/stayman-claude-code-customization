...
## WP-A5 · Cheap discrimination probe — is single-turn replay salvageable? `[PRE-REGISTERED 2026-09-21, not yet run]`

**Operator-approved re-scope (2026-09-21), chosen over building the multi-turn drive loop.**
The drive loop stays **buried** pending this probe's result — it would be attempt #4 at this
bug class, and three prior attempts died without a stop-condition.

### The mechanical cause — FOUND AND CONFIRMED 2026-09-21, before any runs were spent

**My first hypothesis was WRONG, and the negative control is what caught it.** I proposed that
`cd /tmp` left the `Skill` tool unable to *resolve* `feature-verify-human` (no `.claude/skills/`
there). Testing a dot-free probe cwd with skills symlinked in listed all four `feature-verify-*`
skills — but so did **bare `/tmp`**, because skills resolve from *user-global* `~/.claude/skills/`
regardless of cwd. `/tmp` never lacked them. Had I only run the positive arm, I would have
"confirmed" a false cause and rebuilt the harness around it.

**The actual cause is the permission mode.** `tools/replay-baseline.sh:311` passes
`--permission-mode dontAsk`, which **blocks the `Skill` tool outright**:

> `Permission to use Skill has been denied because Claude Code is running in don't ask mode.`

Verified as a clean pair from the same cwd, same model, same `--disallowed-tools`:

| `--permission-mode` | `Skill(feature-verify-human)` |
|---|---|
| `dontAsk` (WP-A3's setting) | **DENIED** — tool blocked by mode |
| `bypassPermissions` | **`SKILL_CALL_OK`** |

**So the model could never chain in any of WP-A3's 100 runs.** "Hand back" was the only
*permitted* action in both arms — a hard floor under stops and controls alike, and a sufficient
mechanical explanation for controls firing at 57.5%. The instrument was measuring its own
permission configuration.

This is the `[Phase 20]` lesson again, one level up: **sensitivity to the variable you thought
to vary is not evidence about the one you did not.** WP-A3 varied the slice and the n; it never
varied the permission mode, so the mode's effect was invisible to the entire matrix. It is also
a second instance of this cycle's own governing lesson — the negative control, not the positive
result, is what produced the finding.

### Method

- **2 slices only**, the two extremes from WP-A3: `chained-hermes-b` (control, 65%) and
  `stop-hermes-a` (stop, 10%). If discrimination is recoverable at all, it shows here.
- **n=20 each**, opus-5 pinned (per-scenario opt-out of the 4.7 memory, operator-confirmed
  2026-09-21 — see `.claude/memory/feedback_replay_harness_research_conditions.md`).
- **`--permission-mode bypassPermissions`** in place of `dontAsk` — the single variable under
  test, and the confirmed cause above. This is what makes "did it chain?" a *real observation*
  rather than a declared intention: the model can now actually call the tool.
  - Keep `--disallowed-tools "Edit,Write,NotebookEdit"` — the runs must stay non-mutating.
    `bypassPermissions` widens what is *permitted*, so the write-blocking list is now
    **load-bearing rather than belt-and-braces**; do not drop it.
  - Keep the **dot-free `/tmp/claude-replay-<uuid>` cwd** per the 4.7 memory's still-active
    rule. It is not the variable under test, but it costs nothing to honour and the memory's
    cwd clause was never opted out of.
- **Re-classify on tool_use, not on prose.** With `Skill` unblocked, "chained" becomes a
  *structural* observation — did a `Skill` tool_use block appear? Per root `CLAUDE.md`:
  classify structurally, never on prose keywords. The existing `classify()` reads prose and
  must be updated, or the probe inherits the bug it is trying to escape.
- **Drop runs that never emit F10b** (16/100 in WP-A3, which bucketed at 88% edge-pause and
  were measuring "the model did something else entirely"). These are not observations of this
  bug. Report both the filtered and unfiltered tallies.
- Est. ~$10, ~1 hour. No new tooling — a `--cwd` flag on the existing runner.

### Gate — PRE-REGISTERED AND BINDING

Written **before** the run. Deciding this bar after seeing numbers is how a null result gets
narrated into a win (the WP-B1 warning, applied to the instrument itself).

| outcome | reading | action |
|---|---|---|
| **stops > controls, CIs non-overlapping** | the missing skills dir WAS the confound; single-turn replay is salvageable | scope the full re-run; Track A lives |
| **controls ≥ stops** | single-turn replay is **DEAD** for this bug class — not the permission mode, something inherent | **archive Track A. Do NOT build the drive loop.** Route to WP-B3 |
| **both arms collapse toward 0%** | the mode was the *whole* signal; there is no bug left to see at this depth | not a pass — the harness now lacks sensitivity. Re-scope, do not A/B against it |
| ambiguous (CIs overlap, stops nominally higher) | underpowered, not a pass | **treat as FAIL** for the binding decision; a wider n is a *new* pre-registration, not a rescue of this one |

**Report ratios (`13/20`), never verdicts.** Per `docs/lessons/green-tests-that-guard-nothing.md`
(eighth mechanism): n≥6 is the floor for any claim, and a mutation that silently fails to mutate
is indistinguishable from an inert assertion — so **verify the skills dir is actually resolvable
from the probe cwd before trusting a single run**, or the probe tests nothing and looks like it
passed.
