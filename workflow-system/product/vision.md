...
- **Human-in-the-loop:** At every real decision point (verify-human, triage severity, plan review) the workflow pauses the conversation and waits for the operator rather than guessing or silently proceeding.
...
7. **Human interruptibility is a feature.** The workflow pauses at named human-input points and waits — it never silently blocks on the user or barrels past a decision that is the operator's to make.
