...
2. **Agent-side guard is CONTEXTUAL — keyed on workflow position, not universal.** Whether to write `.session.md` depends on where in the workflow you are, not on a blanket "always confirm":
   ...
   - **The pause-policy table is authoritative for the chaining decision (this prose is the human-readable statement).** The boundary auto-chain is modeled in the state machine as the **"Session-boundary exit chain"** pause-policy block (`workflow-system/product/transitions.md` "Drive modes" + all 4 `agents/*/AGENTS.md` cheat-sheet tables), with the full `finalize → reflect → [capture] → handoff` chain and its two modeled edges: **`S22` (reflect → session-handoff**, no-learning arm) and **`S23` (session-capture → session-handoff**, learning-found arm, after the save lands). Both are AUTO in all drive modes at a clean boundary; the mid-workflow-ambiguity CONFIRM above is the narrow exception. When driving, read the table row — not this prose. Modeled 2026-07-21 (`SURFACE-...-BOUNDARY-HANDOFF-AUTOCHAIN-NOT-IN-STATE-MACHINE`), pinned by `tests/check-structure.sh` [Phase 18].
   - **Capture-gate is drive-mode-conditional (AC-6, part of the same feature).** On the learning-found arm, `session-capture`'s confirmation gate depends on drive mode + scope: in **autopilot/FSD** a **`[PROJECT]`-scope** learning **auto-writes** (surfaced in chat as a read-time veto — path + content + scope printed before the `git amend`, so a `git reset` is the operator's veto), while a **`[GLOBAL]`-scope** learning **still confirms** even in autopilot/FSD (higher blast radius — every logged reflect scope-correction was `[GLOBAL]`→`[PROJECT]`). Modes 1/2 always confirm (unchanged). This is what lets the boundary exit chain auto-run to the handoff for the common project-scope case without a blind global write.
